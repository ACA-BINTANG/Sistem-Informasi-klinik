<?php
    ...
    if (isset($_POST['add_user'])) {
        $id = generateID("USR");
        $un = $_POST['username']; $em = $_POST['email']; $ps = $_POST['password']; $rl = $_POST['role']; $nm = $_POST['nama_lengkap'];
        mysqli_query($conn, "INSERT INTO userm VALUES ('$id', '$un', '$em', '$ps', '$rl', '$nm')");
        header("Location: adminMaster.php?page=user&msg=User Berhasil Ditambah"); exit;
    }

    // 2. TAMBAH STAFF + AUTO AKUN SSO
    if (isset($_POST['add_staff'])) {
        $nip = $_POST['no_identitas']; $nama = $_POST['nama_lengkap']; $role = $_POST['role_akun']; 
        $id_u = generateID("USR"); $id_s = generateID("STF");
        $username_sso = $nip . "@polytechnic.astar.ac.id"; 
        $nama_depan = strtolower(explode(' ', trim($nama))[0]);
        $pass_staff = $nama_depan . "123";
        mysqli_query($conn, "INSERT INTO userm VALUES ('$id_u', '$username_sso', '$username_sso', '$pass_staff', '$role', '$nama')");
        $jbt = $_POST['jabatan']; $ins = $_POST['instansi']; $npa = $_POST['npa_idi']; $hp = $_POST['no_hp'];
        mysqli_query($conn, "INSERT INTO staffm VALUES ('$id_s', '$id_u', '$nama', '$nip', '$jbt', '$ins', '$npa', '$hp')");
        header("Location: adminMaster.php?page=staff&msg=Staff Berhasil Didaftarkan"); exit;
    }
    ...
    if (isset($_POST['add_pasien'])) {
        ...
        if(mysqli_query($conn, $q_user) && mysqli_query($conn, $q_pasien)) {
            header("Location: adminMaster.php?page=pasien&msg=Pasien & Akun SSO Berhasil Dibuat!");
            exit;
        }
    }
// 4. UPDATE USER (Jika User diupdate, maka Staff/Pasien yang terhubung ikut berubah)
    if (isset($_POST['update_user'])) {
        ...
        mysqli_query($conn, "UPDATE userm SET username='$un', email='$em', password='$ps', role='$rl', nama_lengkap='$nm' WHERE id_user='$id'");

        // Sinkronisasi ke tabel staffm (berdasarkan id_user)
        mysqli_query($conn, "UPDATE staffm SET no_identitas='$un', nama_lengkap='$nm' WHERE id_user='$id'");

        // Sinkronisasi ke tabel pasienm (berdasarkan id_user)
        mysqli_query($conn, "UPDATE pasienm SET no_identitas='$un', nama_pasien='$nm' WHERE id_user='$id'");

        header("Location: adminMaster.php?page=user&msg=Akun & Data Terelasi Berhasil Diupdate"); exit;
    }
    ...
    if (isset($_POST['update_staff'])) {
        ...
        // Update tabel staffm
        mysqli_query($conn, "UPDATE staffm SET nama_lengkap='$nm_l', no_identitas='$no_i', jabatan='$jbt', instansi='$ins', npa_idi='$npa', no_hp='$hp' WHERE id_staff='$id_s'");

        // Ambil id_user yang terkait dengan staff ini untuk update userm
        $get_user = mysqli_query($conn, "SELECT id_user FROM staffm WHERE id_staff='$id_s'");
        ...
        mysqli_query($conn, "UPDATE userm SET username='$no_i', email='$new_email', nama_lengkap='$nm_l' WHERE id_user='$id_u'");

        header("Location: adminMaster.php?page=staff&msg=Data Staff & Akun SSO Berhasil Diupdate"); exit;
    }
    ...
    if (isset($_POST['update_pasien'])) {
        ...
        mysqli_query($conn, "UPDATE pasienm SET nama_pasien='$nm', no_identitas='$nip', jenis_kelamin='$jk', kategori_pasien='$kat', unit_prodi='$prodi', alamat='$alm', no_hp='$hp' WHERE id_pasien='$id_p'");

        // Ambil id_user yang terkait dengan pasien ini
        $get_user_p = mysqli_query($conn, "SELECT id_user FROM pasienm WHERE id_pasien='$id_p'");
        ...
        mysqli_query($conn, "UPDATE userm SET username='$nip', email='$new_email_p', nama_lengkap='$nm' WHERE id_user='$id_u_p'");

        header("Location: adminMaster.php?page=pasien&msg=Data Pasien & Akun SSO Berhasil Diupdate"); exit;
    }
    
    // 7. HAPUS UNIVERSAL
    if (isset($_GET['del'])) {
        $tabel = $_GET['t']; $kolom = $_GET['k']; $val = $_GET['del']; $pg = $_GET['page'];
        mysqli_query($conn, "SET FOREIGN_KEY_CHECKS = 0");
        mysqli_query($conn, "DELETE FROM $tabel WHERE $kolom = '$val'");
        mysqli_query($conn, "SET FOREIGN_KEY_CHECKS = 1");
        header("Location: adminMaster.php?page=$pg&msg=Data Berhasil Dihapus"); exit;
    }
    ...
    $chart_roles = mysqli_query($conn, "SELECT role, COUNT(*) as jumlah FROM userm GROUP BY role");
    $chart_labels = []; $chart_data = [];
    while($row = mysqli_fetch_assoc($chart_roles)) { $chart_labels[] = $row['role']; $chart_data[] = $row['jumlah']; }
    ?>
    ...
    <div class="sidebar">
        ...
        <nav class="nav flex-column gap-1">
            <a class="nav-link <?= ($active_page == 'dashboard') ? 'active' : '' ?>" href="?page=dashboard"><i class="bi bi-speedometer2"></i> Overview</a>
            <div class="nav-group-title">Manajemen Akun</div>
            <a class="nav-link <?= ($active_page == 'user') ? 'active' : '' ?>" href="?page=user"><i class="bi bi-person-lock"></i> User Credentials</a>
            <a class="nav-link <?= ($active_page == 'staff') ? 'active' : '' ?>" href="?page=staff"><i class="bi bi-shield-check"></i> Tim Pengelola</a>
            <a class="nav-link <?= ($active_page == 'pasien') ? 'active' : '' ?>" href="?page=pasien"><i class="bi bi-people"></i> Database Pasien</a>
        </nav>
    </div>

<main class="main-content">
    <?php if(isset($_GET['msg'])): ?><div class="alert alert-success border-0 shadow-sm mb-4 rounded-4 fw-bold text-center"><i class="bi bi-check-circle-fill me-2"></i> <?= $_GET['msg'] ?></div><?php endif; ?>

    <?php if($active_page == 'dashboard'): ?>
        <!-- Dashboard Tetap Sama -->
        <h3 class="fw-bold mb-4">Dashboard Overview</h3>
        <div class="row g-4 mb-5">
            <div class="col-md-4"><div class="stat-card"><div><div class="small fw-bold text-muted">PASIEN</div><div class="h2 fw-bold text-primary"><?= mysqli_num_rows($p_list) ?></div></div><i class="bi bi-people fs-1 opacity-25"></i></div></div>
            <div class="col-md-4"><div class="stat-card" style="border-left-color: #f6c23e;"><div><div class="small fw-bold text-muted">TIM STAFF</div><div class="h2 fw-bold text-warning"><?= mysqli_num_rows($s_list) ?></div></div><i class="bi bi-shield-lock fs-1 opacity-25"></i></div></div>
            <div class="col-md-4"><div class="stat-card" style="border-left-color: #1cc88a;"><div><div class="small fw-bold text-muted">OBAT</div><div class="h2 fw-bold text-success"><?= mysqli_num_rows(mysqli_query($conn, "SELECT * FROM obatm")) ?></div></div><i class="bi bi-capsule fs-1 opacity-25"></i></div></div>
        </div>
    ...
    <?php elseif($active_page == 'user'): ?>
        ...
        <div class="data-container"><div class="table-responsive"><table class="table table-hover align-middle">
            ...
            <tbody id="tableUser">
            <?php $no=1; while($row = mysqli_fetch_assoc($u_list)): ?>
                <tr class="user-row" data-role="<?= $row['role'] ?>">
                    ...
                    <td><span class="badge bg-info bg-opacity-10 text-info px-3 py-2 rounded-pill small"><?= $row['role'] ?></span></td>
                    <td>
                        <button class="btn btn-sm btn-light text-warning me-1" data-bs-toggle="modal" data-bs-target="#mEditU<?= $row['id_user'] ?>"><i class="bi bi-pencil-square"></i></button>
                        <a href="?del=<?= $row['id_user'] ?>&t=userm&k=id_user&page=user" class="btn btn-sm btn-light text-danger" onclick="return confirm('Hapus user?')"><i class="bi bi-trash3"></i></a>
                    </td>
                </tr>
    ...
    <?php elseif($active_page == 'staff'): ?>
        ...
        <div class="data-container"><div class="table-responsive"><table class="table table-hover align-middle">
            ...
            <tbody id="tableStaff">
            <?php $no=1; while($row = mysqli_fetch_assoc($s_list)): ?>
                <tr class="staff-row" data-instansi="<?= $row['instansi'] ?>">
                    <td class="text-muted small"><?= $no++ ?></td>
                    <td class="fw-bold"><?= $row['no_identitas'] ?></td>
                    <td class="nama-staff"><?= $row['nama_lengkap'] ?></td>
                    <td><small class="fw-bold"><?= $row['jabatan'] ?></small></td>
                    <td><span class="badge bg-light text-dark border"><?= $row['instansi'] ?></span></td>
                    <td><small class="text-success fw-bold">+62 <?= $row['no_hp'] ?? '-' ?></small></td>
                    <td>
                        <button class="btn btn-sm btn-light text-warning me-1" data-bs-toggle="modal" data-bs-target="#mEditS<?= $row['id_staff'] ?>"><i class="bi bi-pencil-square"></i></button>
                        <a href="?del=<?= $row['id_staff'] ?>&t=staffm&k=id_staff&page=staff" class="btn btn-sm btn-light text-danger" onclick="return confirm('Hapus staff?')"><i class="bi bi-trash3"></i></a>
                    </td>
                </tr>
            <?php endwhile; ?></tbody></table></div></div>
    ...
    <?php elseif($active_page == 'pasien'): ?>
        ...
        <div class="data-container"><div class="table-responsive"><table class="table table-hover align-middle">
            ...
            <?php $no=1; while($row = mysqli_fetch_assoc($p_list)): ?>
                <tr class="pasien-row" data-prodi="<?= $row['unit_prodi'] ?>">
                    <td class="text-muted small"><?= $no++ ?></td>
                    <td class="fw-bold text-primary"><?= $row['no_identitas'] ?></td>
                    <td class="nama-pasien"><?= $row['nama_pasien'] ?></td>
                    <td><span class="badge bg-secondary opacity-75"><?= $row['kategori_pasien'] ?></span></td>
                    <td><small class="fw-bold"><?= $row['unit_prodi'] ?></small></td>
                    <td><small class="text-success fw-bold">+62 <?= $row['no_hp'] ?? '-' ?></small></td>
                    <td>
                        <button class="btn btn-sm btn-light text-warning me-1" data-bs-toggle="modal" data-bs-target="#mEditP<?= $row['id_pasien'] ?>"><i class="bi bi-pencil-square"></i></button>
                        <a href="?del=<?= $row['id_pasien'] ?>&t=pasienm&k=id_pasien&page=pasien" class="btn btn-sm btn-light text-danger" onclick="return confirm('Hapus pasien?')"><i class="bi bi-trash3"></i></a>
                    </td>
                </tr>
            <?php endwhile; ?></tbody></table></div></div>
    <?php endif; ?>
    ...
    <!-- MODAL EDIT USER -->
    <?php mysqli_data_seek($u_list, 0); while($u = mysqli_fetch_assoc($u_list)): ?>
    <div class="modal fade" id="mEditU<?= $u['id_user'] ?>" tabindex="-1"><div class="modal-dialog modal-dialog-centered"><form class="modal-content border-0 shadow-lg" style="border-radius: 20px;" method="POST">
        ...
        <div class="modal-body p-4">
            ...
            <label class="small fw-bold">Username</label><input type="text" name="username" class="form-control mb-3 bg-light border-0" value="<?= $u['username'] ?>">
            <label class="small fw-bold">Email</label><input type="email" name="email" class="form-control mb-3 bg-light border-0" value="<?= $u['email'] ?>">
            <label class="small fw-bold">Password</label><input type="text" name="password" class="form-control mb-3 bg-light border-0" value="<?= $u['password'] ?>">
            <label class="small fw-bold">Nama</label><input type="text" name="nama_lengkap" class="form-control mb-3 bg-light border-0" value="<?= $u['nama_lengkap'] ?>">
            <select name="role" class="form-select bg-light border-0"><option <?=($u['role']=='Dokter')?'selected':''?>>Dokter</option><option <?=($u['role']=='Pasien')?'selected':''?>>Pasien</option><option <?=($u['role']=='K3')?'selected':''?>>K3</option></select>
    ...
    <!-- MODAL EDIT STAFF -->
    <?php mysqli_data_seek($s_list, 0); while($s = mysqli_fetch_assoc($s_list)): ?>
    <div class="modal fade" id="mEditS<?= $s['id_staff'] ?>" tabindex="-1"><div class="modal-dialog modal-dialog-centered"><form class="modal-content border-0 shadow-lg" style="border-radius: 20px;" method="POST">
        ...
        <div class="modal-body p-4">
            ...
            <input type="text" name="no_identitas" class="form-control mb-2 bg-light border-0" value="<?= $s['no_identitas'] ?>">
            <input type="text" name="nama_lengkap" class="form-control mb-2 bg-light border-0" value="<?= $s['nama_lengkap'] ?>">
            <input type="text" name="jabatan" class="form-control mb-2 bg-light border-0" value="<?= $s['jabatan'] ?>">
            <select name="instansi" class="form-select mb-2 bg-light border-0"><option <?=($s['instansi']=='Kampus')?'selected':''?>>Kampus</option><option <?=($s['instansi']=='Siloam')?'selected':''?>>Siloam</option></select>
            <input type="text" name="npa_idi" class="form-control mb-2 bg-light border-0" value="<?= $s['npa_idi'] ?>">
            <div class="input-group"><span class="input-group-text bg-light border-0">+62</span><input type="text" name="no_hp" class="form-control bg-light border-0 phone-mask" value="<?= $s['no_hp'] ?? '' ?>"></div>
    ...
    <!-- MODAL EDIT PASIEN -->
    <?php mysqli_data_seek($p_list, 0); while($p = mysqli_fetch_assoc($p_list)): ?>
    <div class="modal fade" id="mEditP<?= $p['id_pasien'] ?>" tabindex="-1"><div class="modal-dialog modal-dialog-centered"><form class="modal-content border-0 shadow-lg" style="border-radius: 20px;" method="POST">
        ...
        <div class="modal-body p-4">
            ...
            <input type="text" name="no_identitas" class="form-control mb-2 bg-light border-0" value="<?= $p['no_identitas'] ?>">
            <input type="text" name="nama_pasien" class="form-control mb-2 bg-light border-0" value="<?= $p['nama_pasien'] ?>">
            <select name="jenis_kelamin" class="form-select mb-2 bg-light border-0"><option value="L" <?=($p['jenis_kelamin']=='L')?'selected':''?>>Laki-laki</option><option value="P" <?=($p['jenis_kelamin']=='P')?'selected':''?>>Perempuan</option></select>
            <select name="kategori_pasien" class="form-select mb-2 bg-light border-0"><option <?=($p['kategori_pasien']=='Mahasiswa')?'selected':''?>>Mahasiswa</option><option <?=($p['kategori_pasien']=='Pegawai')?'selected':''?>>Pegawai</option><option <?=($p['kategori_pasien']=='Tamu')?'selected':''?>>Tamu</option></select>
            ...
            <select name="unit_prodi" class="form-select bg-light border-0 mb-2">
                ...
                $kategori_unit = [
                    "Program Studi (Mahasiswa)" => [
                        "MI" => "D3 - Manajemen Informatika", "MK" => "D3 - Mekatronika", "MO" => "D3 - Mesin Otomotif", "P4" => "D3 - Pembuatan Peralatan Presisi", "TPM" => "D3 - Teknik Produksi & Proses Manufaktur", "TKBG" => "D4 - Teknologi Konstruksi Bangunan Gedung", "TRL" => "D4 - Teknologi Rekayasa Logistik", "TRPAB" => "D4 - Teknologi Rekayasa Pemeliharaan Alat Berat", "TRPL" => "D4 - Teknologi Rekayasa Perangkat Lunak"
                    ],
                    "Unit / Divisi Kerja (Pegawai)" => [
                        "BAA" => "Biro Administrasi Akademik (BAA)", "BAK" => "Biro Administrasi Keuangan (BAK)", "BKM" => "Biro Kemahasiswaan & Alumni", "WKS" => "Workshop & Laboratorium Pusat", "HRD" => "Human Resources (HRD / Kepegawaian)", "IT" => "IT Support & Digital Systems", "GA" => "General Affair (Sarpras & Fasilitas)", "DIR" => "Sekretariat Direktorat", "K3" => "Departemen K3 & Lingkungan", "SECURITY" => "Divisi Keamanan & Ketertiban"
                    ]
                ];
                foreach ($kategori_unit as $label_grup => $opsi_unit) {
                    echo "<optgroup label='$label_grup'>";
                    foreach ($opsi_unit as $key => $val) {
                        $is_selected = ($p['unit_prodi'] == $key) ? 'selected' : '';
            ...
            <input type="text" name="alamat" class="form-control mb-2 bg-light border-0" value="<?= $p['alamat'] ?>">
            <div class="input-group"><span class="input-group-text bg-light border-0">+62</span><input type="text" name="no_hp" class="form-control bg-light border-0 phone-mask" value="<?= $p['no_hp'] ?? '' ?>"></div>
    ...
    <script>
        ...
        if(ctx) {
            new Chart(ctx, { type: 'bar', data: { labels: <?= json_encode($chart_labels) ?>, datasets: [{ label: 'Jumlah Akun', data: <?= json_encode($chart_data) ?>, backgroundColor: '#0057B8', borderRadius: 10 }] }, options: { plugins: { legend: { display: false } }, scales: { y: { beginAtZero: true, grid: { display: false } }, x: { grid: { display: false } } } } });
        }
