...
<body>
  ...
  <script>
    ...
    // Jangan tampilkan border merah saat pengguna sedang mengetik atau berpindah kolom.
    // Border merah hanya diberikan setelah tombol Login ditekan dan validasi gagal.
    Object.entries(fields).forEach(([field, input]) => {
      input.addEventListener('input', () => {
        if (!input.classList.contains('is-invalid')) return;

        // Setelah submit gagal, border merah boleh hilang ketika nilai sudah diperbaiki,
        // tetapi tidak pernah ditambahkan kembali hanya karena pengguna sedang mengetik.
        let message = '';
        if (field === 'username') {
          const value = username.value.trim();
          if (!value) message = 'Username wajib diisi. Masukkan NIM, NIP, email, atau username akun Anda.';
          else if (value.length > 100) message = 'Username terlalu panjang. Kurangi hingga maksimal 100 karakter.';
        }
        if (field === 'password') {
          if (!password.value) message = 'Kata sandi wajib diisi. Masukkan kata sandi akun Anda.';
          else if (password.value.length > 72) message = 'Kata sandi terlalu panjang. Kurangi hingga maksimal 72 karakter.';
        }

        if (!message) setError(field, '');
      });
    });
