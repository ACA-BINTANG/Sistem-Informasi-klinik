...
$totalAnggaran = 0;
$rekapStatus = [
    "Pending" => ["qty" => 0, "nilai" => 0],
    "Proses" => ["qty" => 0, "nilai" => 0],
    "Diterima" => ["qty" => 0, "nilai" => 0],
    "Batal" => ["qty" => 0, "nilai" => 0],
];
...
while ($row = mysqli_fetch_assoc($qPengadaan)) {
    ...
    if (isset($rekapStatus[$st])) {
        $rekapStatus[$st]["qty"] += (int) $row["jumlah_order"];
        $rekapStatus[$st]["nilai"] += (float) $row["subtotal"];
    }
}

// Anggaran yang sudah benar-benar terealisasi (obat diterima) vs masih berjalan
$anggaranRealisasi = $rekapStatus["Diterima"]["nilai"];
$anggaranBerjalan =
    $rekapStatus["Pending"]["nilai"] + $rekapStatus["Proses"]["nilai"];
...
<body>
    ...
    <div class="box-ringkasan">
        <table>
            ...
            <tr>
                <td><b>Total Anggaran (Seluruh Status)</b></td>
                <td class="nilai-besar">: <?= e(rupiah($totalAnggaran)) ?></td>
                <td><b>Anggaran Sudah Terealisasi</b> (Diterima)</td>
                <td class="nilai-besar">: <?= e(
                    rupiah($anggaranRealisasi),
                ) ?></td>
            </tr>
            <tr>
                <td><b>Anggaran Masih Berjalan</b> (Pending/Proses)</td>
                <td class="nilai-besar">: <?= e(
                    rupiah($anggaranBerjalan),
                ) ?></td>
                <td></td>
                <td></td>
            </tr>
    ...
    <table class="rekap-status">
        ...
        <tbody>
            <?php foreach ($rekapStatus as $st => $val): ?>
                <tr>
                    <td><?= e($st) ?></td>
                    <td class="text-center">
                        <?php
                        $countSt = 0;
                        foreach ($dataPengadaan as $r) {
                            if (($r["status"] ?: "Pending") === $st) {
                                $countSt++;
                            }
                        }
                        echo e($countSt);
                    ...
                    <td class="text-center"><?= e($val["qty"]) ?></td>
                    <td class="text-right"><?= e(rupiah($val["nilai"])) ?></td>
                </tr>
            <?php endforeach; ?>
        </tbody>
    </table>
    ...
    <div class="penutup">
        Demikian laporan pengadaan dan anggaran obat ini kami sampaikan sebagai bahan rekonsiliasi
