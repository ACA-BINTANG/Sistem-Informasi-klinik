...
if ($id_pasien == '') {
    die("ID pasien tidak ditemukan. Pastikan akun pasien terhubung dengan tabel pasienm.");
}
...
if (isset($_POST['batal_booking'])) {
    ...
    if ($id_rm_batal == '') {
        header("Location: dashboard_pasien.php?page=beranda&err=Data booking tidak ditemukan");
        exit;
    }

    $update_batal = mysqli_query($conn, "
        UPDATE rekam_medis
        SET status = 'Batal'
    ...
    if (!$update_batal) {
        header("Location: dashboard_pasien.php?page=beranda&err=" . urlencode(mysqli_error($conn)));
        exit;
    }

    if (mysqli_affected_rows($conn) > 0) {
        header("Location: dashboard_pasien.php?page=beranda&msg=Booking berhasil dibatalkan");
        exit;
    } else {
        header("Location: dashboard_pasien.php?page=beranda&err=Booking tidak bisa dibatalkan. Mungkin sudah selesai atau sudah dibatalkan.");
        exit;
    }
}
...
if (isset($_POST['ambil_antrean_jadwal'])) {
    ...
    $jam_booking_raw = mysqli_real_escape_string($conn, $_POST['jam_booking'] ?? '');
    $keluhan         = mysqli_real_escape_string($conn, $_POST['keluhan_booking'] ?? '');

    if ($id_jadwal == '' || $jam_booking_raw == '' || $keluhan == '') {
        header("Location: dashboard_pasien.php?page=jadwal_dokter&err=Data booking belum lengkap. Pilih jadwal, jam, dan isi keluhan.");
        exit;
    }
    ...
    if (!preg_match('/^\d{2}:\d{2}:\d{2}$/', $jam_booking)) {
        header("Location: dashboard_pasien.php?page=jadwal_dokter&err=Format jam booking tidak valid");
        exit;
    }
    ...
    if (!$qJadwal) {
        header("Location: dashboard_pasien.php?page=jadwal_dokter&err=" . urlencode(mysqli_error($conn)));
        exit;
    }

    if (mysqli_num_rows($qJadwal) == 0) {
        header("Location: dashboard_pasien.php?page=jadwal_dokter&err=Jadwal dokter tidak tersedia atau sedang tutup");
        exit;
    }
    ...
    if ($jam_booking < $jam_mulai || $jam_booking >= $jam_selesai) {
        header("Location: dashboard_pasien.php?page=jadwal_dokter&err=Jam booking harus berada di dalam jam buka dokter");
        exit;
    }
    ...
    if ($tgl_kunjungan === false) {
        header("Location: dashboard_pasien.php?page=jadwal_dokter&err=Jam booking sudah lewat. Silakan pilih jam setelah waktu sekarang.");
        exit;
    }
    ...
    if ($cekPasien && mysqli_num_rows($cekPasien) > 0) {
        header("Location: dashboard_pasien.php?page=jadwal_dokter&err=Anda sudah memiliki antrean aktif pada tanggal tersebut");
        exit;
    }

    $cekJam = mysqli_query($conn, "
        SELECT id_rekam_medis
        FROM rekam_medis
    ...
    if ($cekJam && mysqli_num_rows($cekJam) > 0) {
        header("Location: dashboard_pasien.php?page=jadwal_dokter&err=Jam tersebut sudah dipakai pasien lain, silakan pilih jam lain");
        exit;
    }
    ...
    if ($insert) {
        header("Location: dashboard_pasien.php?page=beranda&msg=Booking berhasil dibuat. Nomor antrean Anda $no_baru");
        exit;
    } else {
        header("Location: dashboard_pasien.php?page=jadwal_dokter&err=" . urlencode(mysqli_error($conn)));
        exit;
...
if (isset($_POST['ambil_antrean'])) {
    ...
    if ($keluhan == '') {
        header("Location: dashboard_pasien.php?page=antrean&err=Keluhan wajib diisi");
        exit;
    }

    // Cek apakah ada dokter yang buka hari ini pada jam sekarang
    $qDokterBuka = mysqli_query($conn, "
    ...
    if (!$qDokterBuka) {
        header("Location: dashboard_pasien.php?page=antrean&err=" . urlencode(mysqli_error($conn)));
        exit;
    }

    if (mysqli_num_rows($qDokterBuka) == 0) {
        header("Location: dashboard_pasien.php?page=jadwal_dokter&err=Dokter sedang tidak buka untuk antrean langsung. Silakan booking di Jadwal Dokter.");
        exit;
    }

    $dokterBuka = mysqli_fetch_assoc($qDokterBuka);
    $id_staff_langsung = mysqli_real_escape_string($conn, $dokterBuka['id_staff']);
    ...
    if ($insert) {
        header("Location: dashboard_pasien.php?page=beranda&msg=Antrean langsung $no_baru berhasil diambil!");
        exit;
    } else {
        header("Location: dashboard_pasien.php?page=antrean&err=" . urlencode(mysqli_error($conn)));
        exit;
...
// =======================
$qDiagnosaBooking = mysqli_query($conn, "
...
<div class="sidebar">
    ...
    <nav class="nav flex-column">
        <a class="nav-link <?= ($active_page == 'beranda') ? 'active' : '' ?>" href="dashboard_pasien.php?page=beranda">
            <i class="bi bi-grid-1x2-fill"></i> Dashboard
        </a>

        <a class="nav-link <?= ($active_page == 'antrean') ? 'active' : '' ?>" href="dashboard_pasien.php?page=antrean">
            <i class="bi bi-ticket-perforated-fill"></i> Ambil Antrean
        </a>

        <a class="nav-link <?= ($active_page == 'jadwal_dokter') ? 'active' : '' ?>" href="dashboard_pasien.php?page=jadwal_dokter">
    ...
    <nav class="nav flex-column">
        <a class="nav-link <?= ($active_page == 'riwayat') ? 'active' : '' ?>" href="dashboard_pasien.php?page=riwayat">
            <i class="bi bi-clock-history"></i> Riwayat Berobat
        </a>

        <a class="nav-link <?= ($active_page == 'obat') ? 'active' : '' ?>" href="dashboard_pasien.php?page=obat">
...
<main class="main-content">
    ...
    <?php if($active_page == 'beranda'): ?>
        ...
        <div class="d-flex justify-content-between align-items-center mb-4">
            <h3 class="fw-bold mb-0 text-dark">Halo, <?= e(explode(' ', $pasien_name)[0]) ?>!</h3>
            ...
            <div class="col-lg-5">
                <?php 
                    ...
                    if($q_my && mysqli_num_rows($q_my) > 0): 
                        ...
                        $posisi_antrian = hitungPosisiAntrean($conn, $d_my['id_rekam_medis'], $d_my['tgl_kunjungan']);
                ?>
                    <?php if($posisi_antrian == 1 && $d_my['tgl_kunjungan'] == date('Y-m-d')): ?>
                        ...
                        <div class="stat-card">
                            <div>
                                ...
                                <div class="h2 fw-bold text-primary mb-0">
                                    <?= mysqli_num_rows(mysqli_query($conn, "SELECT id_rekam_medis FROM rekam_medis WHERE id_pasien='$id_pasien' AND status='Selesai'")) ?>
                                </div>
                            </div>
    ...
    <?php elseif($active_page == 'antrean'): ?>
        ...
        <?php
            ...
            $jam_info = date('H:i:s');
            $cekJadwalInfo = mysqli_query($conn, "
                SELECT *
                FROM jadwalm
                ...
                LIMIT 1
            ");
            $dokter_sedang_buka = ($cekJadwalInfo && mysqli_num_rows($cekJadwalInfo) > 0);
        ?>
    ...
    <?php elseif($active_page == 'jadwal_dokter'): ?>
        ...
        <div class="row g-4">
            ...
            if (!$qJadwal) {
                echo "<div class='col-12'><div class='alert alert-danger rounded-4 border-0 shadow-sm'>Query error: " . e(mysqli_error($conn)) . "</div></div>";
            } else {
                if (mysqli_num_rows($qJadwal) == 0) {
                    echo "
                        <div class='col-12'>
                    ...
                    <div class="col-md-4">
                        <div class="card jadwal-card h-100">
                            <div class="jadwal-date">
                                <div class="jadwal-day"><?= e($j['tanggal']) ?></div>
                            ...
                            <div class="card-body p-4">
                                ...
                                <div class="mb-3">
                                    ...
                                    <div class="jadwal-time"><?= e(substr($j['jam_mulai'], 0, 5)) ?> - <?= e(substr($j['jam_selesai'], 0, 5)) ?></div>
                                </div>

                                <button class="btn btn-primary w-100 rounded-4 fw-bold py-2" data-bs-toggle="modal" data-bs-target="#mBooking<?= e($j['id_jadwal']) ?>">
        ...
        <?php foreach($modal_booking as $j): ?>
            <div class="modal fade" id="mBooking<?= e($j['id_jadwal']) ?>" tabindex="-1">
                    ...
                    <form class="modal-content border-0 shadow-lg" style="border-radius:24px" method="POST">
                        ...
                        <div class="modal-body p-4">
                            ...
                            <div class="mb-3">
                                ...
                                <input type="text" class="form-control bg-light border-0 py-3" value="<?= e($j['tanggal']) ?>, <?= e(substr($j['jam_mulai'], 0, 5)) ?> - <?= e(substr($j['jam_selesai'], 0, 5)) ?>" readonly>
                            </div>
                            ...
                            <div class="mb-3">
                                ...
                                <input type="time"
                                       ...
                                       min="<?= e(substr($j['jam_mulai'], 0, 5)) ?>"
                                       max="<?= e(date('H:i', strtotime($j['jam_selesai']) - 60)) ?>"
                                       required>
                                <div class="form-text small text-muted">
                                    Pilih jam sesuai jam buka dokter. Tanggal kunjungan otomatis mengikuti hari <strong><?= e($j['tanggal']) ?></strong> terdekat.
                                </div>
                            </div>
            ...
            <div class="table-responsive">
                <table class="table table-hover align-middle">
                    ...
                    <tbody>
                        ...
                        ?>
                            <tr>
                                <td class="fw-bold text-primary"><?= e($ro['nama_obat']) ?></td>
                                <td><span class="badge bg-light text-dark border px-3"><?= e($ro['satuan'] ?? 'Umum') ?></span></td>
                                <td class="text-center">
                                    <span class="badge <?= ($ro['stok_sekarang'] > 0) ? 'bg-success' : 'bg-danger' ?> bg-opacity-10 text-<?= ($ro['stok_sekarang'] > 0) ? 'success' : 'danger' ?> px-4 py-2 rounded-pill fw-bold">
                ...
                <table class="table table-hover align-middle">
                    ...
                    <tbody>
                        ...
                        ?>
                            <tr>
                                <td>
                                    <div class="fw-bold small"><?= e(date('d M Y', strtotime($row['tgl_kunjungan']))) ?></div>
                                    <small class="text-muted"><i class="bi bi-clock me-1"></i><?= e(substr($row['waktu_booking'], 0, 5)) ?></small>
                                </td>
                                <td><span class="badge bg-light text-dark border"><?= e($row['jenis_antrean'] ?? 'Langsung') ?></span></td>
                                <td><div style="max-width: 250px;" class="small text-truncate" title="<?= e($row['keluhan']) ?>"><?= e($row['keluhan']) ?></div></td>
                                <td><span class="badge <?= ($row['status']=='Selesai') ? 'bg-primary bg-opacity-10 text-primary' : 'bg-warning bg-opacity-10 text-warning' ?> px-3"><?= e($row['nama_penyakit'] ?? 'N/A') ?></span></td>
                                <td class="text-center">
                                    <button class="btn btn-sm btn-light border rounded-pill px-3 shadow-sm fw-bold" data-bs-toggle="modal" data-bs-target="#mDetail<?= e($row['id_rekam_medis']) ?>">
                                        <i class="bi bi-eye me-1"></i>Detail
                                    </button>
                                </td>
                            </tr>

                            <div class="modal fade" id="mDetail<?= e($row['id_rekam_medis']) ?>" tabindex="-1">
                                <div class="modal-dialog modal-dialog-centered">
                                    <div class="modal-content border-0 shadow-lg" style="border-radius: 20px;">
                                        <div class="modal-header bg-light border-0 p-4">
                                            <h6 class="fw-bold mb-0">Catatan Medis: <?= e(date('d/m/Y', strtotime($row['tgl_kunjungan']))) ?></h6>
                                            <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                                        </div>
                                        <div class="modal-body p-4 text-start">
                                            <label class="small fw-bold text-muted text-uppercase mb-1">Jenis Antrean:</label>
                                            <h6 class="fw-bold mb-3"><?= e($row['jenis_antrean'] ?? 'Langsung') ?></h6>
                                            <label class="small fw-bold text-muted text-uppercase mb-1">Diagnosa:</label>
                                            <h5 class="text-primary fw-bold"><?= e($row['nama_penyakit'] ?? 'Menunggu Pemeriksaan') ?></h5>
                                            <hr>
                                            ...
                                            <div class="p-3 bg-light rounded-3 text-muted" style="font-size: 13.5px;">
                                                <?= nl2br(e($row['hasil_pemeriksaan'] ?? 'Dokter belum mengisi catatan medis untuk antrean ini.')) ?>
                                            </div>
                                        </div>
                                    </div>
                                </div>
...
<?php if(isset($posisi_antrian) && $posisi_antrian == 1 && isset($d_my) && $d_my['tgl_kunjungan'] == date('Y-m-d')): ?>
setTimeout(function() {
