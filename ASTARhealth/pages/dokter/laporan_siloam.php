...
<div class="data-container mb-4">
    <form method="GET" class="row g-3">
        ...
        <div class="col-md-4">
            ...
            <input class="form-control" name="search" placeholder="Cari obat / supplier / ID" value="<?= e($_GET['search'] ?? '') ?>">
        </div>
        ...
        <div class="col-md-2">
            ...
            <select class="form-select" name="status">
                ...
                <?php foreach (['Pending','Proses','Diterima','Batal'] as $st): ?>
...
<?php renderReportDonutChart('Chart Status Pengadaan', 'Grafik bulat dari data pengadaan yang sudah difilter.', $chartRows, 'label', 'total'); ?>
            ...
            <tbody>
                ...
                <?php if (count($rows) > 0): ?>
                    <?php foreach ($rows as $r): ?>
                        <tr>
                            <td><?= $no++ ?></td>
                            <td class="fw-bold text-primary"><?= e($r['id_pengadaan']) ?></td>
                            <td><?= e($r['tgl_order']) ?></td>
                            <td><?= e($r['nama_obat'] ?? '-') ?></td>
                            <td><?= e($r['nama_supplier'] ?? '-') ?></td>
                            <td><?= e($r['jumlah_order']) ?> <?= e($r['satuan'] ?? '') ?></td>
                            <td><span class="badge bg-primary"><?= e($r['status']) ?></span></td>
                        </tr>
                    <?php endforeach; ?>
                <?php else: ?>
